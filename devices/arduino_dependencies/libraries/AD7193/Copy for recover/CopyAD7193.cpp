...
bool AD7193::begin(void) {
  SPI.begin();
  SPI.beginTransaction(SPISettings(4920000, MSBFIRST, SPI_MODE3));
...
void AD7193::WaitForADC(void)  {
    ...
    Serial.print("\t Waiting for Conversion ");
    ...
    while(1){
      ...
      if (breakTime > 3000) {                       // Break after 3 seconds - avoids program hanging up
...
unsigned long AD7193::ReadADCChannelsData(void)  {
  
    unsigned char byteIndex = 0;
    unsigned long buffer = 0;
    unsigned char receiveBuffer = 0;
    unsigned char dataLength = 4;  // data length depends on if Status register is appended to Data read - see AppendStatusValuetoData()

    WaitForADC();

    SPI.transfer(0x5C);  // command to start read data in continous mode
...
void AD7193::SetChannels(int channels) {

    // generate Channel settings bits for Configuration write
    unsigned long channelBits = channels << 8;
...
void AD7193::SetContinousConversionMode(void) {
  ...
  registerMap[1] &= 0x1FFFFF; //keep all bit values except Channel bits
  registerMap[1] |= 0x100000; // Continous conversion mode bits and DAT_STA bit enabled 
...
void AD7193::SetSincFilter(int sinc)  {

  Serial.print("\nSetting Sinc Filter 3 to ");
  Serial.println(sinc);
  
  unsigned long SincFilterSetting;

  if(sinc == 0)         {SincFilterSetting = 0x0;}
  else if (sinc == 1)   {SincFilterSetting = 1<<15;}
  else {
    Serial.println("\tERROR - Invalid Sinc Filter Setting - no changes made.  Valid Sinc Filter settings are Sinc3 (sinc=1) or Sinc4 (sinc=0)");
    return;
  }

  int regAddress = AD7193_REG_MODE;

  registerMap[regAddress] &= 0xFF7FFF; //keep all bit values except gain bits
  registerMap[regAddress] |= SincFilterSetting;
