...
def main():
    ...
    reward_fct_descriptor_2024_04_01 = "exp_pre_3_exp_10"
    reward_fct_descriptor_2024_04_10 = "reward_2024_04_10"
    ...
    prefactor_goal = 100
    prefactor_fail = 10
    save_name = (f"TQC_reward_2024_04_22_betas_step_{beta_step}_goal_{beta_goal_1}"
                 f"_{beta_goal_2}_alphas_step_{alpha_step}_goal_{alpha_goal}_prefactors_{prefactor_step}"
                 f"_{prefactor_fail}_{prefactor_goal}"
                 f"_hyperparameter_search_fail.csv")
    if os.path.exists(save_name):
        df = pd.read_csv(save_name, index_col=0)
    else:
        df = pd.DataFrame(data=None, index=None,
                      columns=["beta_fail_1", "beta_fail_2", "alpha_fail", "timestamp",
                               "number_timesteps", "percentage_in_goal", "percentage_failed",
                               "P_max_episode_steps_ave"])
    # change different alphas, betas fail reward
    a_list = [[0.1, 1, 1], [0.1, 1, 5], [0.1, 5, 1], [0.5, 1, 1], [0.5, 1, 5], [0.5, 5, 1]]
    for _ in range(5):
        for a in a_list:
            beta_fail_1 = a[1]
            beta_fail_2 = a[2]
            ...
            df = pd.concat([df, pd.DataFrame([[beta_fail_1, beta_fail_2, alpha_fail,
