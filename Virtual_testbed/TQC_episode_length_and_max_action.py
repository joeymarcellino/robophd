...
def main():
    ...
    save_name = (f"TQC_reward_2024_04_22_betas_{beta_step}_{beta_fail_1}_{beta_fail_2}_{beta_goal_1}"
                 ...
                 f"_{prefactor_fail}_{prefactor_goal}_episode_length_max_action_in_steps.csv")

    # data frame for information needed
    if os.path.exists(save_name):
        df = pd.read_csv(save_name, index_col=0)
    else:
        df = pd.DataFrame(data=None, index=None,
                          columns=["max_episode_steps", "max_actioninsteps", "timestamp",
                                   "number_timesteps",
                                   "percentage_in_goal", "percentage_failed", "P_max_episode_steps_ave"])
    for _ in range(5):
        # compare different episode lengths and max actions
        for max_episode_steps in [30, 40]:
            for max_actioninsteps in [2*10**3, 6*10**3, 10**4]:
                ...
                df = pd.concat([df, pd.DataFrame([[max_episode_steps,max_actioninsteps,
                                                   timestamp, num, percentage_in_goal, percentage_failed,
                                                   P_max_episode_steps_ave]], columns=df.columns)],
                               ignore_index=True)
                df.to_csv(save_name)
                for i in range(10):
                    # learn for 10k training steps
