...
def main():
    ...
    save_name = (f"TQC_reward_2024_04_22_betas_{beta_step}_{beta_fail_1}_{beta_fail_2}_{beta_goal_1}"
                 f"_{beta_goal_2}_alphas_{alpha_step}_{alpha_fail}_{alpha_goal}_prefactor_step_{prefactor_step}"
                 f"_hyperparameter_search_prefactors.csv")
    if os.path.exists(save_name):
        df = pd.read_csv(save_name, index_col=0)
    else:
        df = pd.DataFrame(data=None, index=None,
                          columns=["prefactor_fail", "prefactor_goal", "alpha_step", "timestamp",
                                   "number_timesteps",
                                   "percentage_in_goal", "percentage_failed", "P_max_episode_steps_ave"])
    # iterate through different prefactors in the reard
    for _ in range(5):
        for prefactor_fail in [prefactor_step, prefactor_step * 10, prefactor_step * 100]:
            for prefactor_goal in [prefactor_step, prefactor_step * 10, prefactor_step * 100]:
                ...
                df = pd.concat([df, pd.DataFrame([[prefactor_fail, prefactor_goal, alpha_step,
