...
def main():
    ...
    # compare different reset methods
    for reset_method, number_episode_to_neutral in [["move_power_up", 1], ["move_power_up", 10], ["interval", 1]]:
        dir_names = f"reset_tests/" + reset_method
        if reset_method == "interval":
            dir_names += str(initial_radius)
        if reset_method == "move_power_up":
            dir_names += str(number_episode_to_neutral)
