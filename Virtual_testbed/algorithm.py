...
def main():
    ...
    for algorithm, alg_descriptor in [[TQC, "TQC"], [SAC, "SAC"], [TD3, "TD3"], [DDPG, "DDPG"], [PPO, "PPO"], [A2C, "A2C"], [CrossQ, "CrossQ"]]:
        ...
        elif alg_descriptor == "TD3" or alg_descriptor == "DDPG":
            n_actions = env.action_space.shape[-1]
            action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=0.1 * np.ones(n_actions))
            model = algorithm("MlpPolicy", env, action_noise=action_noise, verbose=1, tensorboard_log=env.logdir)
        else:
            model = algorithm("MlpPolicy", env, verbose=1, tensorboard_log=env.logdir)
        TIMESTEPS = 10000
        for i in range(50):
            model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=alg_descriptor,
