...
def main():
    ...
    number_tries = 100
    # dataframe for saving episode length, percentage in goal, ...
    df = pd.DataFrame(data=None, index=None,
                      columns=["episode_length", "timestamp",
                               "number_timesteps",
    ...
    save_name = (f"TQC_reward_2024_04_22_betas_{beta_step}_{beta_fail_1}_{beta_fail_2}_{beta_goal_1}"
                 ...
                 f"_{prefactor_fail}_{prefactor_goal}_episode_length.csv")
    # compare different episode lengths
    for _ in range(5):
        for max_episode_steps in range(5, 51, 5):
