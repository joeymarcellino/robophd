...
def main():
    ...
    save_name = (f"TQC_reward_2024_22_betas_fail_{beta_fail_1}_{beta_fail_2}_goal_{beta_goal_1}"
                 f"_{beta_goal_2}_alphas_fail_{alpha_fail}_goal_{alpha_goal}_prefactors_{prefactor_step}"
                 f"_{prefactor_fail}_{prefactor_goal}"
                 f"_hyperparameter_search_step.csv")
    if os.path.exists(save_name):
        df = pd.read_csv(save_name, index_col=0)
    else:
        df = pd.DataFrame(data=None, index=None,
                          columns=["beta_step", "alpha_step", "timestamp",
                                   "number_timesteps", "percentage_in_goal", "percentage_failed",
                                   "P_max_episode_steps_ave"])
    for _ in range(5):
        for a in [[0.1, 1], [0.9, 1], [0.1, 10], [0.9, 10]]:
            alpha_step = a[0]
            beta_step = a[1]
            ...
            df = pd.concat([df, pd.DataFrame([[beta_step, alpha_step,
            ...
            for i in range(10):
                ...
                for j in range(number_tries):
                    ...
                    if env.fail:
                        percentage_failed += 1 / number_tries
                df = df = pd.concat([df, pd.DataFrame([[beta_step, alpha_step,
